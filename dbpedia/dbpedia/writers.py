...
class WhoWroteQuestion(QuestionTemplate):
    ...
    """
    write_wordnet = synonyms_ref.getLemmaPreposition("write", 'v')
    author_wordnet = synonyms_ref.getLemmaPreposition("author", 'n')
    regex = ((Pos("WP") + write_wordnet + Book()) |
             (Question(Lemmas("who be") + Pos("DT")) +
              author_wordnet + Pos("IN") + Book())) + \
            Question(Pos("."))
...
class BooksByAuthorQuestion(QuestionTemplate):
    ...
    write_wordnet = synonyms_ref.getLemmaPreposition("write", 'v')
    regex = (Question(list_wordnet) + Lemmas("book by") + Author()) | \
            ((Lemma("which") | Lemma("what")) + Lemmas("book do") +
             Author() + write_wordnet + Question(Pos(".")))
